...
class Engine():
    def __init__(self):
        check = requests.post('http://89.104.70.42:8000/api/check_license/', data={'hwid': HWID})
        if check.status_code == 200:
            ...
            if check['status'] == True:
                ...
                if (hashlib.md5((HWID + salt).encode())).hexdigest() != hwid_new:
                    ...
                    pass
                    # check_v = requests.post('http://localhost:8000/api/check_license/',
                    #                         data={'hwid': HWID, 'version': __version__}).json()
                    # if check_v['status']:
                        ...
                        for videoID in videoIds['result']:
                            ...
                            while True:
                                try:
                                    api = random.choice(self.apis)
                                    resp = requests.get(
                                        f'https://www.googleapis.com/youtube/v3/channels?part=statistics%2Csnippet&maxResults=50&id={channelId}&key={str(api)}',
                                        proxies=proxies)
                            ...
                            while True:
                                ...
                                try:
                                    api = random.choice(self.apis)
                                    resp = requests.get(
                                        f"https://youtube.googleapis.com/youtube/v3/videos?part=snippet&part=contentDetails&part=statistics&id={vid}&key={api}",
                                        proxies=proxies)
                                    if resp.status_code == 200:
    ...
    def console_log(self):
        while True:
            ...
            logger.info(
                f'ApiKeys: {len(self.apis)} | KeyWords: {len(self.keys)} | Emails: {self.emails} | Video_seen: {self.videocount}')
            sleep(5)
